...
void print(int q[]) {
   ...
   // Fill board with pointers to bb and wb in alternate positions, which is based on the parity of the sum of the row and column indices.
   for(i=0; i<8; i++)
      for(j=0; j<8; j++)
         if((i+j)%2 == 0)
            board[i][j] = &wb;
